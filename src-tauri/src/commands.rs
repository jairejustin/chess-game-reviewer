...
use crate::engine::uci_engine::{UciEngine, Evaluation};
...
#[tauri::command]
pub fn analyze_game(pgn: String) -> Result<Vec<AnalyzedMove>, String> {
    let mut visitor = PgnVisitor::new();
    let mut reader = Reader::new(Cursor::new(pgn.as_bytes()));
    
    let positions = match reader.read_game(&mut visitor) {
        Ok(Some(p)) => p,
        _ => return Err("Failed to parse PGN".to_string()),
    };
    ...
    for (san, fen) in positions {
        ...
        let (played_eval, best_move, pv) = engine.analyze_position(&fen, depth);
        ...
        let normalized_cp = if ply_count % 2 != 0 { -played_cp } else { played_cp };
        let classification = classify(prev_eval, normalized_cp, best_eval);
