...
async function initAudio() {
  ...
  try {
    ...
    // Fetch only the core Lichess .ogg files in parallel
...
export function playBoardSound(type: SoundType = 'move') {
  ...
  // Fallback to move sound for everything except explicit captures
  ...
  gainNode.gain.value = 0.6; // Keep at a comfortable volume
